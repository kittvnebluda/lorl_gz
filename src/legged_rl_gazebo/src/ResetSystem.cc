                       ...
                       gz::sim::EventManager& _eventManager)
{
    ...
    {
        auto type_comp = _ecm.Component<gz::sim::components::JointType>(joint_entity);
        if (!type_comp)
        {
        ...
        {
            auto name_comp = _ecm.Component<gz::sim::components::Name>(joint_entity);
            if (name_comp)
            {
...
void
ResetSystem::PreUpdate(const gz::sim::UpdateInfo& _info, gz::sim::EntityComponentManager& _ecm)
{
    ...
    for (const auto it : req_joint_map_)
    {
        ...
        _ecm.RemoveComponent<gz::sim::components::JointVelocityCmd>(joint_entity);

        auto pos_reset = _ecm.Component<gz::sim::components::JointPositionReset>(joint_entity);
        if (pos_reset)
        {
        ...
        auto vel_reset = _ecm.Component<gz::sim::components::JointVelocityReset>(joint_entity);
        if (vel_reset)
        {
            vel_reset->Data() = { 0.0 };
        }
        ...
        {
            _ecm.CreateComponent(joint_entity, gz::sim::components::JointVelocityReset({ 0.0 }));
        }
    }
...
void
ResetSystem::onResetRobot(const std::shared_ptr<legged_rl_interfaces::srv::ResetRobot::Request> req,
                          std::shared_ptr<legged_rl_interfaces::srv::ResetRobot::Response> res)
{
    ...
    auto pose =
      gz::math::Vector3d(req->pose.position.x, req->pose.position.y, req->pose.position.z);
